...
(async () => {
  try {
    ...
    for (const materia of materias) {
      ...
      const content = await get('SELECT id, titulo FROM conteudos WHERE materia = ? ORDER BY ordem ASC LIMIT 1', [materia]);
      if (!content) continue;
      ...
      console.log(`Inserted quiz for materia='${materia}' (conteudo_id=${content.id}) as quiz id ${res.id}`);
    }
