...
const AdminPanel: React.FC<{ userRole: string }> = ({ userRole }) => {
  ...
  if (userRole !== "admin") {
    return (
      <div className="text-red-700 font-bold" role="alert">
  ...
  const addQuestion = () =>
    setForm({ ...form, quiz: [...form.quiz, { ...defaultQuestion }] });
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded shadow mt-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block font-semibold" htmlFor="videoUrl">
            Link do Vídeo
          </label>
          <input
            id="videoUrl"
        ...
        <div>
          <label className="block font-semibold" htmlFor="explanation">
            Explicação
          </label>
          <textarea
            id="explanation"
            ...
            required
            aria-required="true"
            aria-label="Explicação do conteúdo"
          />
        </div>
        <div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-2">
                {q.options.map((opt, oidx) => (
                  <input
                    ...
                    value={opt}
                    onChange={(e) =>
                      handleQuizChange(idx, String(oidx), e.target.value)
                    }
                    className="border rounded px-2 py-1"
                    required
                    aria-required="true"
                    aria-label={`Opção ${oidx + 1} da pergunta ${idx + 1}`}
                  />
                ))}
              </div>
              <div>
                <label className="block" htmlFor={`answer-${idx}`}>
                  Resposta Correta
                </label>
                <select
                  id={`answer-${idx}`}
                ...
                >
                  {q.options.map((_, oidx) => (
                    <option key={oidx} value={oidx}>{`Opção ${
                      oidx + 1
                    }`}</option>
                  ))}
                </select>
