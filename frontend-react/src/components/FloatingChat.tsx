  ...
  const testTutorGeneration = async () => {
    try {
      ...
      const res = await fetch(`/api/tutor`, {
        ...
        body: JSON.stringify({ pergunta: 'Teste do tutor: diga Olá de forma breve.' })
      });
      const elapsed = performance.now() - started;
      setLastLatencyMs(Math.round(elapsed));
      if (!res.ok) {
        setLastStatus(`HTTP ${res.status}`);
        setMessages(prev => [...prev, { from: 'tutor', text: `Teste: falha (HTTP ${res.status}).` }]);
      } else {
        const data = await res.json();
        const answer = typeof data?.answer === 'string' ? data.answer : 'Sem resposta do tutor.';
        const details = data?.context?.error ? ` (detalhe: ${String(data.context.error).slice(0,120)})` : '';
        setLastStatus('OK');
        setMessages(prev => [...prev, { from: 'tutor', text: `Teste: ${answer}${details}` }]);
      }
    } catch (e: any) {
      setLastStatus(e?.message || 'Erro de rede');
      setMessages(prev => [...prev, { from: 'tutor', text: `Teste: ${e?.message || 'Falha de conexão.'}` }]);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const data = await res.json();
        const answer = typeof data?.answer === 'string' ? data.answer : 'Sem resposta do tutor.';
        setMessages(prev => [...prev, { from: 'tutor', text: cleanText(answer) }]);
      }
    } catch (e: any) {
      setMessages(prev => [...prev, { from: 'tutor', text: e?.message || 'Falha de conexão com a API.' }]);
    } finally {
      setSending(false);
    }
  };
        ...
        >
          <div className="bg-bridgeBlue-700 text-white p-3 rounded-t-xl flex justify-between items-center">
            <span id="chat-title" className="flex items-center gap-2">
              Tutor Gemini
              <span
                className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded ${tutorOk ? 'bg-green-600' : 'bg-red-600'}`}
                aria-label={`Status do tutor: ${tutorOk ? 'online' : 'offline'}`}
              >
                <span className="w-2 h-2 rounded-full bg-white inline-block" />
                {tutorOk ? 'online' : 'offline'}
              </span>
              {tutorModel && (
                <span className="text-xs bg-white/10 px-2 py-0.5 rounded" aria-label={`Modelo do tutor: ${tutorModel}`}>{tutorModel}</span>
              )}
              {lastLatencyMs !== null && (
                <span className="text-xs bg-white/10 px-2 py-0.5 rounded" aria-label={`Latência do último teste: ${lastLatencyMs}ms`}>
          ...
          >
            {messages.map((msg, idx) => (
              ...
              >
                <div
                  className={`px-3 py-2 rounded-lg max-w-sm whitespace-pre-wrap ${msg.from === 'user' ? 'bg-bridgeTeal-500/20 text-gray-900' : 'bg-gray-200 dark:bg-gray-800 dark:text-gray-100'}`}
                  aria-label={`${msg.from === 'user' ? 'Você' : 'Tutor'} diz: ${msg.text}`}
                >
                  {msg.text}
                </div>
              </div>
          ...
          <div className="p-2 border-t flex">
            ...
            >{checkingTutor ? 'Testando...' : 'Testar Tutor'}</button>
            <button
