      ...
      try {
        ...
        if (tok) {
          ...
          if (parts.length === 3) {
            ...
            setUserName(typeof payload?.nome === 'string' ? payload.nome : null);
            setUserRole(typeof payload?.cargo === 'string' ? payload.cargo : null);
            return;
          }
        }
        setUserName(null);
    ...
    <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 sticky top-0 z-40">
      <div className="container-page py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-gray-900 dark:text-gray-100">Ponte do Saber</span>
          {userName && (
        ...
        <div className="flex items-center gap-2">
          <NavLink
            to="/aluno"
            className={({ isActive }) => `btn px-3 py-2 ${isActive ? 'bg-bridgeBlue-700 text-white' : 'btn-outline'}`}
          >Aluno</NavLink>
          <NavLink
            to="/trilhas"
            className={({ isActive }) => `btn px-3 py-2 ${isActive ? 'bg-bridgeBlue-700 text-white' : 'btn-outline'}`}
          ...
          {userName && (
            <>
              <button onClick={logout} className="btn px-3 py-2 btn-outline" aria-label="Sair">Sair</button>
            </>
          )}
          <button
            onClick={toggleTheme}
