...
const UsersAdmin: React.FC = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data?.error || `Falha ao listar usuários (HTTP ${res.status})`
        );
      setUsers(data);
    } catch (e: any) {
  ...
  const promote = async (u: User) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data?.error || `Falha ao promover (HTTP ${res.status})`
        );
      await fetchUsers();
  ...
  const removeUser = async (u: User) => {
    const ok = confirm(
      `Excluir usuário ${u.nome} (${u.email})? Esta ação não pode ser desfeita.`
    );
    if (!ok) return;
    try {
      const res = await fetch(`/api/usuarios/${u.id}`, {
