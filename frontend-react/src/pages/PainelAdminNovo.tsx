...
const PainelAdminNovo: React.FC = () => {
  ...
  const [selectedConteudoId, setSelectedConteudoId] = useState<number | null>(null);
  ...
  const loadQuizzes = async (conteudoId: number) => {
    ...
    try {
      const r = await fetch(`/api/admin/quizzes?conteudoId=${conteudoId}`, { headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) } });
      const j = await r.json();
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rq = await fetch('/api/quizzes', {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) },
        body: JSON.stringify({ conteudo_id: conteudoId, pergunta, op_a: opA, op_b: opB, op_c: opC, op_d: opD, correta })
      });
      const jq = await rq.json();
      if (!rq.ok) throw new Error(jq?.error || 'Falha ao criar quiz');
      setMsg('Vídeo e quiz cadastrados com sucesso.');
      setTitulo(''); setUrl(''); setPergunta(''); setOpA(''); setOpB(''); setOpC(''); setOpD(''); setCorreta('A');
      load();
    } catch (err: any) {
  ...
  const excluir = async (id: number) => {
    try {
      const r = await fetch(`/api/conteudos/${id}`, { method: 'DELETE', headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) } });
      const j = await r.json();
  ...
  const editarQuiz = async (q: any) => {
    try {
      ...
      const r = await fetch(`/api/quizzes/${q.id}`, {
        ...
        body: JSON.stringify({ pergunta: perguntaNovo, op_a, op_b, op_c, op_d, correta })
      });
      const j = await r.json();
  ...
  const excluirQuiz = async (q: any) => {
    ...
    if (!ok) return;
    try {
      const r = await fetch(`/api/quizzes/${q.id}`, { method: 'DELETE', headers: { ...(token ? { 'Authorization': `Bearer ${token}` } : {}) } });
      const j = await r.json();
  ...
  return (
    <div className="container-page py-8">
      <div className="card p-6">
        <h1 className="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Painel Admin</h1>
        {msg && <div className="mb-3 text-sm text-blue-700 dark:text-blue-300">{msg}</div>}
        <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label className="block text-sm mb-1">Categoria</label>
            <select className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={materia} onChange={e=>setMateria(e.target.value)}>
          ...
          <div>
            ...
            <input className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={titulo} onChange={e=>setTitulo(e.target.value)} />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm mb-1">URL do YouTube</label>
            <input className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={url} onChange={e=>setUrl(e.target.value)} />
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm mb-1">Pergunta do Quiz</label>
            <input className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={pergunta} onChange={e=>setPergunta(e.target.value)} />
          </div>
          {['A','B','C','D'].map((k, idx)=> (
            <div key={k}>
              <label className="block text-sm mb-1">Opção {k}</label>
              <input className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={[opA,opB,opC,opD][idx]} onChange={e=>{
                const v=e.target.value; if(k==='A') setOpA(v); if(k==='B') setOpB(v); if(k==='C') setOpC(v); if(k==='D') setOpD(v);
              }} />
            </div>
          ))}
          <div>
            <label className="block text-sm mb-1">Correta</label>
            <select className="w-full border rounded px-3 py-2 bg-white dark:bg-gray-800 dark:text-gray-100" value={correta} onChange={e=>setCorreta(e.target.value as any)}>
          ...
          <button type="submit" className="btn btn-primary md:col-span-2">Salvar</button>
        </form>
              ...
              <tr key={it.id} className="border-t">
                ...
                <td className="py-2">
                  <div className="flex gap-2">
                    <button className="btn btn-outline" onClick={()=>loadQuizzes(it.id)}>Gerenciar Quiz</button>
                    <button className="btn btn-outline" onClick={()=>excluir(it.id)}>Excluir</button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {selectedConteudoId && (
          <div className="mt-6">
            <h2 className="text-lg font-semibold mb-2">Quizzes do Conteúdo #{selectedConteudoId}</h2>
            {loadingQuizzes ? (
              <div className="text-sm text-gray-600">Carregando…</div>
            ) : quizzes.length === 0 ? (
              <div className="text-sm text-gray-600">Nenhuma pergunta cadastrada.</div>
            ) : (
                  ...
                  <tbody>
                    {quizzes.map((q:any) => (
                      <tr key={q.id} className="border-b">
                        ...
                        <td className="py-2 pr-3 max-w-xl truncate" title={q.pergunta}>{q.pergunta}</td>
                        ...
                        <td className="py-2 pr-3">
                          <div className="flex gap-2">
                            <button className="btn btn-outline" onClick={()=>editarQuiz(q)}>Editar</button>
                            <button className="btn btn-outline" onClick={()=>excluirQuiz(q)}>Excluir</button>
                          </div>
                        </td>
                      </tr>
                    ))}
