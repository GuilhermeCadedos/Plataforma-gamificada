...
const Profile: React.FC = () => {
  ...
  const handleProfilePictureUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files && event.target.files[0]) {
      ...
      try {
        ...
        console.debug('Uploading profile picture — token length:', token.length, 'startsWithBearer?:', /^Bearer\s+/i.test(localStorage.getItem('token')||''));
        if (!token) {
        ...
        if (response.ok) {
          ...
          try { localStorage.setItem('profilePicture', stored || ''); } catch(_) {}
          ...
          if (response.status === 401 || (data && data.error && data.error.toLowerCase().includes('token'))) {
            alert('Token inválido ou expirado. Você será redirecionado para a página de login.');
    ...
    (async () => {
      try {
        ...
        if (j?.nome || j?.name) {
          ...
          try { localStorage.setItem('userName', j.nome || j.name); } catch {}
        }
        if (j?.foto_perfil) {
          const resolved = resolveStoredUrl(j.foto_perfil);
          setProfilePicture(resolved);
          try { localStorage.setItem('profilePicture', resolved || ''); } catch(_) {}
  ...
  return (
    <div className="container-page py-8">
      <h1 className="text-3xl font-bold mb-6 text-gray-900">Perfil do Usuário</h1>
        ...
        <div className="card p-6">
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <div className="mt-3">
                <label htmlFor="profileUpload" className="btn-upload">Carregar foto</label>
                <input id="profileUpload" type="file" accept="image/*" onChange={handleProfilePictureUpload} style={{ display: 'none' }} />
              </div>
            </div>
          </div>
        </div>
