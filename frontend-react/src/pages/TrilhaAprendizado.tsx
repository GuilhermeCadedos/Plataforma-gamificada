  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      <div className="container-page py-8">
        <h1 className="text-2xl font-semibold mb-4 text-gray-900 dark:text-gray-100">Trilha de Aprendizado</h1>
            ...
            {Object.entries(agrupados).map(([materia, items]) => (
              <section key={materia}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {items.map(c => {
                    const vid = c.tipo === 'video' ? extractYouTubeId(c.url) : null;
                    const embed = vid ? `https://www.youtube.com/embed/${vid}` : null;
                    return (
                        ...
                        {c.tipo === 'video' ? (
                          embed ? (
                            <div className="aspect-video mb-3">
                              <iframe src={embed} title={c.titulo} className="w-full h-full" allowFullScreen />
                            </div>
                          ) : (
                            <p className="text-sm text-gray-500 mb-3">Vídeo inválido</p>
                          )
                        ) : (
                          <p className="text-sm text-gray-700 mb-3">{c.explicacao}</p>
                        )}

                        <div className="flex items-center justify-between">
                          <button className="btn bg-blue-600 text-white px-3 py-1 rounded" onClick={() => concluir(c.id)}>Concluir</button>
                          {typeof c.xp === 'number' && <span className="text-sm">XP: {c.xp}</span>}
                        </div>

                        {xpAwarded[c.id] !== undefined && <div className="mt-2 text-green-700">+{xpAwarded[c.id]} XP</div>}
                      </article>
                    );
                  })}
                </div>
